...
def _ensure_agent_cost_callback(agent: Any) -> None:
    """Ensure the given agent has a step-completed callback that records costs and prints CoT.

    This is a best-effort fallback when a higher-level system hasn't attached its own callback yet. 
    We attach a local callback that prints reasoning traces (CoT) and a short cost summary.
    """
    try:
        ...
        def _local_cb(trace: dict):
            try:
                ...
                # --- START: Chain of Thought Printing ---
                # Check for reasoning_content (DeepSeek/OpenAI standards from model.py)
                reasoning = msg.get("reasoning_content") or msg.get("reasoning")
                
                # Handle list format if raw object, or string if serialized
