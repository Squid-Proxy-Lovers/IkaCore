...
class Message:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # --- FIX START: Serialize reasoning for DeepSeek ---
        if self.reasoning:
            # DeepSeek requires the field 'reasoning_content'
            d["reasoning_content"] = "\n".join(self.reasoning)
        # --- FIX END ---
...
class OpenAIModel(Model):
    ...
    def generate(
        ...
        **kwargs
        ...
        reasoning = getattr(choice, "reasoning", None) # gpt reasoning
        reasoning_content = getattr(choice, "reasoning_content", getattr(choice, "reasoning", None)) # DeepSeek reasoning

        
        if reasoning_content:
            _LOG.info("CoT Reasoning Trace:\n%s", reasoning_content)
        return Message(
...
class DeepSeekAPIModel(OpenAIModel):
    ...
    def _get_model_cost(self, model_id):
        return {
            ...
            "deepseek-reasoner": (0.14, 0.14, 0.28),
