...
@tool
def smb_download_file(target_ip: str, share: str, remote_path: str, local_path: str) -> str:
    """
...
@tool
def windows_password_policy(target_ip: str) -> str:
    """
    Attempts to retrieve Windows password policy using enum4linux (if installed).
...
def kerberoast_detect_only(domain: str, dc_ip: str, username: str, password: str) -> str:
    """
    Detection-only placeholder for Kerberoasting opportunities.
...
def _wrap_all(funcs: list) -> list:
    return [function_to_ika_tool(f) for f in funcs]
