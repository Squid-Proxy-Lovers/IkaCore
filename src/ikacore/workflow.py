...
from agents import ParaBaseAgent, summarise_message_history


WorkflowCompressionHook = Callable[[List[str], ParaBaseAgent], str]
...
class WorkflowNode:
    name: str
    agent: ParaBaseAgent
    stage_wiring: Optional[Dict[int, Dict[str, List[ParaBaseAgent]]]] = None
...
class AsyncWorkflowExecutor:
    ...
    def schedule_node(self, node_name: str, runner: "ParaWorkflow") -> None:
...
class ParaWorkflow:
    ...
    def __repr__(self) -> str:
        lines = []
        lines.append(f"ParaWorkflow: {self.name}")
    ...
    # ------------------------------------------------------------------
    def _default_compress_hook(self, contexts: List[str], agent: ParaBaseAgent) -> str:
