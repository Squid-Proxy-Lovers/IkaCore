...
class StageEndToolMixin:
    @staticmethod
    def _stage_end_tool() -> AgentTool:
        return AgentTool(
            id="stage_end",
            name="stage_end",
...
class StageChangeToolMixin(StageEndToolMixin):
    @staticmethod
    def _change_stage_tool(allowed_stages_str: str) -> AgentTool:
        return AgentTool(
            id="change_stage",
            name="change_stage",
...
class StageHitlToolMixin(StageChangeToolMixin):
    @staticmethod
    def _ask_user_tool() -> AgentTool:
        return AgentTool(
            id="ask_user",
            name="ask_user",
...
class IkaStage(StageHitlToolMixin):
    def __init__(
        self,
        name: str,
        prompt: str,
        ...
        self.name = name
        self.prompt = prompt
        self.tools: List[IkaTools | AgentTool] = list(tools)
        self.stage_max_step = stage_max_step
        self.subagents: list[Any] = subagents or []
        ...
        self._append_control_tools()

    def _append_control_tools(self) -> None:
        self.tools.append(self._stage_end_tool())
        if self.allowed_back_to:
            allowed_stages_str = ", ".join(str(idx) for idx in self.allowed_back_to)
            self.tools.append(self._change_stage_tool(allowed_stages_str))

        if self.hitl:
            self.tools.append(self._ask_user_tool())
