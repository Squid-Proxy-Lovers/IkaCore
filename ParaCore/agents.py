...
class ParaBaseAgent:
    def __init__(
        ...
        subagents:list["ParaBaseAgent"] = None, # do we need to create a manger agent class
        next_agent: "ParaBaseAgent" = None, # agent to call after this agent completes 
        bidirectional:bool = False, # allow for feedback loops
        feedback_agent: "ParaBaseAgent" =None, # agent to receive feedback from
        maxsteps:int =10, # maximum number of steps to take
        step_timeout:int = 900, # timeout for each step, default 900 seconds / 15 minutes
        RAGSource: list[type[SquidRAGSource]] = None,  # list of RAG source types to use
        memory: bool = False,
        memory_finder: "SquidMemorySystem" = None,
        ):
        self.name = name
        ...
        if len(self.Stages) > 0:
            if ((len(self.subagents) > 0) or (self.next_agent is not None) or (self.bidirectional is True)):
            ...
            if ((len(self.subagents) > 0) ^ (self.next_agent is not None) ^ (self.bidirectional is True)):
