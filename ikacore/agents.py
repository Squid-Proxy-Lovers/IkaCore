...
class ParaBaseAgent:
    def __init__(
        ...
        self.api_url = api_url if api_url else self._find_api_url(model_id)
    ...
    @staticmethod
    def _find_api_url(model_id: str) -> str:
    ...
    def _agent_end_tool(self) -> AgentTool:
        return AgentTool(
            id="agent_end",
            name="agent_end",
    ...
    def _build_stage_system_prompt(self, stage: SquidStage) -> str:
        parts = [self.start_prompt, stage.prompt, self.end_prompt]
        if not any(parts):
            parts = [self.description, stage.prompt]
        return "\n\n".join([p for p in parts if p])

    def _build_stage_agent_tools(self, stage: SquidStage) -> List[AgentTool]:
        ...
        return stage_tools + subagent_tools + [self._agent_end_tool()]

    def _create_barebone_model(self, system_prompt: str, agent_tools: List[AgentTool]) -> BareBoneModel:
        model = BareBoneModel(
    ...
    def _parse_control_calls(self, tool_calls: List[dict], stage: Optional[SquidStage], current_stage_idx: int = 0) -> tuple[Optional[int], bool, Optional[str]]:
        """Returns (target_stage, agent_end_called, agent_end_text)."""
        target_stage = None
    ...
    def _run_stage(self, stage_index: int, remaining_steps: int) -> tuple[int, str, bool, Optional[str], int]:
        stage = self.Stages[stage_index]
        system_prompt = self._build_stage_system_prompt(stage)
        self.message_history["system"]["message"] = system_prompt

        agent_tools = self._build_stage_agent_tools(stage)
        barebone_model = self._create_barebone_model(system_prompt, agent_tools)
        ...
        for _ in range(step_limit):
            response = chat(barebone_model, messages, self.message_history)
            self.message_history = response.get("message_history", self.message_history)
            last_content = response.get("content", "")
            tool_calls = response.get("tool_calls", []) or []
            used_steps += 1

            target_stage, agent_end_called, agent_end_text = self._parse_control_calls(tool_calls, stage, stage_index)
            if agent_end_called:
    ...
    def _run_simple_agent(self) -> tuple[str, str]:
        agent_tools = self._convert_tools_to_agent_tools(self.tools) + self._convert_subagents_to_tools() + [self._agent_end_tool()]
        ...
        for _ in range(self.maxsteps):
            ...
            _, agent_end_called, agent_end_text = self._parse_control_calls(tool_calls, None)
            if agent_end_called:
    ...
    def execution(self) -> Dict[str, str]:
        ...
        if self.Stages:
            ...
            while 0 <= stage_idx < len(self.Stages) and remaining_steps > 0:
                stage_idx, last_content, agent_end_called, end_text, used = self._run_stage(stage_idx, remaining_steps)
                remaining_steps -= used
                if agent_end_called:
                    agent_end_text = end_text
                    break
            final_message = agent_end_text or last_content
            barebone_model = self._create_barebone_model(self.message_history["system"]["message"], [])
            return self._build_final_output(final_message, barebone_model)

        if self.next_agent:
            final_message, _ = self._run_simple_agent()
            barebone_model = self._create_barebone_model(self.system_prompt or self.description or self.prompt, [])
